<template>
  ...
  <div v-else class="page">
      ...
      <div class="card card-fit">
        ...
        <div v-if="!editingInsights" class="insight-summary">
          ...
          <span v-else class="summary-provider off">
            <Sparkles :size="13" />
            {{ isDemo ? 'Not available in the demo' : 'Off — no provider key' }}
            <!-- A disabled button carrying the explanation in a tooltip was no explanation
                 at all: disabled controls swallow hover on most browsers. -->
            <InfoHint v-if="isDemo" text="Insights are generated by a real backend calling a provider you configure. The demo runs entirely in your browser, so there is nothing to call — self-host Atlas to use your own provider." />
          </span>
          <button v-if="!isDemo" class="btn-small" @click="editingInsights = true">
        ...
        <table class="mappings-table sources-table">
          <tbody>
            <tr v-for="s in sources" :key="s.dataOrigin + s.recordingMethod">
              ...
              <td>
                <button v-if="s.quarantinedCount" class="held-badge" @click="openHeld(s)"
                        title="See which workouts are being held">
    ...
    <!-- A count alone asks you to take Atlas's word for it before enabling a source. Seeing the
         workouts also makes a drifting source obvious: the same session appears again and again
         at slightly different times. -->
    <div v-if="heldSource" class="modal-backdrop" @click.self="closeHeld">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="held-title">
        <h3 id="held-title">Workouts held from {{ heldSource.dataOrigin }}</h3>
        <p class="section-desc">
          Received but not logged, because this source is not enabled. Enabling it adds all of
          them; nothing is lost while they wait.
        </p>

        <p v-if="heldLoading" class="empty-line">Loading…</p>
        <table v-else-if="heldEntries.length" class="mappings-table held-table">
          <tbody>
            <tr v-for="e in heldEntries" :key="e.id">
              <td>{{ e.activity }}<span class="hc-code">{{ e.type }}</span></td>
              <td class="source-seen">{{ formatHeldTime(e.startTime) }}</td>
              <td class="source-seen">{{ e.durationMinutes }} min</td>
            </tr>
          </tbody>
        </table>
        <p v-else class="empty-line">Nothing held.</p>

        <div class="modal-actions">
          <button class="btn-small" @click="enableFromModal">Enable this source</button>
          <button class="btn-small btn-danger" @click="closeHeld">Close</button>
        </div>
      </div>
    </div>
  </div>
...
const heldSource = ref(null)
const heldEntries = ref([])
const heldLoading = ref(false)

async function openHeld(source) {
  heldSource.value = source
  heldEntries.value = []
  heldLoading.value = true
  try {
    heldEntries.value = (await getHeldEntries(source.dataOrigin, source.recordingMethod)).data
  } catch {
    sourceError.value = 'Could not load held workouts'
...
async function enableFromModal() {
  const source = heldSource.value
  closeHeld()
  await enableSource(source)
}

/** Held entries carry the raw ISO string the sender sent, not a parsed date. */
function formatHeldTime(startTime) {
  if (!startTime) return '—'
  const parsed = new Date(startTime)
  if (Number.isNaN(parsed.getTime())) return startTime
  return `${formatDateBr(parsed.toISOString().slice(0, 10))} ${String(parsed.getHours()).padStart(2, '0')}:${String(parsed.getMinutes()).padStart(2, '0')}`
}
...
.modal h3 { margin: 0 0 var(--space-2); font-size: 1rem; }
.held-table { width: 100%; margin-bottom: var(--space-3); }
.modal-actions {
  display: flex;
  gap: 8px;
